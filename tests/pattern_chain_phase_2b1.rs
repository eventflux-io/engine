...
fn test_2b1_0_single_step() {
    ...
    chain.first_processor.lock().unwrap().process(Some(Box::new(event)));
    chain.update_state();
    ...
    assert_eq!(outputs.len(), 1, "Expected 1 output for single-step pattern");
}
...
fn test_2b1_1_simple_two_step() {
    ...
    println!("  PreA has_state_changed before: {}", chain.pre_processors[0].lock().unwrap().has_state_changed());
    let result_a = chain.pre_processors[0].lock().unwrap().process(Some(Box::new(event_a)));
    println!("  Result A: {:?}", result_a.is_some());
    println!("  PreA has_state_changed after: {}", chain.pre_processors[0].lock().unwrap().has_state_changed());
    ...
    chain.update_state();
    println!("  After update_state (propagates forwarded states), outputs = {}", collector.get_outputs().len());
    ...
    println!("  PreB has_state_changed before: {}", chain.pre_processors[1].lock().unwrap().has_state_changed());
    let result_b = chain.pre_processors[1].lock().unwrap().process(Some(Box::new(event_b)));
    println!("  Result B: {:?}", result_b.is_some());
    println!("  PreB has_state_changed after: {}", chain.pre_processors[1].lock().unwrap().has_state_changed());
    chain.update_state();
    println!("  After update_state, outputs = {}", collector.get_outputs().len());
    ...
    assert_eq!(state_event.stream_events.len(), 2, "Expected 2 stream event positions");

    // Verify first position has event A
    assert!(state_event.stream_events[0].is_some(), "Expected event at position 0 (step A)");
    let event_a_result = state_event.stream_events[0].as_ref().unwrap();
    assert_eq!(event_a_result.timestamp, 1000, "Expected timestamp 1000 for event A");

    // Verify second position has event B
    assert!(state_event.stream_events[1].is_some(), "Expected event at position 1 (step B)");
    let event_b_result = state_event.stream_events[1].as_ref().unwrap();
    assert_eq!(event_b_result.timestamp, 2000, "Expected timestamp 2000 for event B");
}
...
fn test_2b1_2_count_quantifiers() {
    ...
    chain.pre_processors[0].lock().unwrap().process(Some(Box::new(event_a1)));
    chain.update_state();
    ...
    chain.pre_processors[0].lock().unwrap().process(Some(Box::new(event_a2)));
    chain.update_state();
    ...
    chain.pre_processors[1].lock().unwrap().process(Some(Box::new(event_b1)));
    chain.update_state();
    ...
    chain.pre_processors[1].lock().unwrap().process(Some(Box::new(event_b2)));
    chain.update_state();
    ...
    assert_eq!(event_a_chain.timestamp, 1000, "Expected first A timestamp 1000");
    ...
    assert_eq!(event_b_chain.timestamp, 3000, "Expected first B timestamp 3000");
...
fn test_2b1_4_wrong_stream_fails() {
    ...
    chain.pre_processors[0].lock().unwrap().process(Some(Box::new(event_c)));
    chain.update_state();
...
fn test_2b1_5_expecting_b_got_a() {
    ...
    let outputs = collector.get_outputs();
    println!("Final outputs count: {}", outputs.len());
    assert_eq!(outputs.len(), 0, "Expected no output when expecting B but got A");
}
...
fn test_2b1_6_range_quantifier() {
    ...
    assert!(outputs.len() <= 2, "Expected at most 2 output events for A{{1,3}}");
